...
#### 线程-局部握手 (Thread-Local Handshakes)
...
#### 移除 Native-Header 自动生成工具 (Remove the Native-Header Generation Tool (javah))

自 Java 9 以来便开始了一些对 JDK 的调整，用户每次调用 javah 工具时会被警告该工具在未来的版本中将会执行的删除操作。当编译 JNI 代码时，已不再需要单独的 Native-Header 工具来生成头文件，因为这可以通过 Java 8（JDK-7150368）中添加的 javac 来完成。

#### 额外的 Unicode 语言标签扩展 (Additional Unicode Language-Tag Extensions)
...
#### 备用存储装置上的堆分配 (Heap Allocation on Alternative Memory Devices)
...
#### 基于 Java 的 实验性 JIT 编译器 (Experimental Java-Based JIT Compiler)
...
#### 基于时间的版本发布模式 (Time-Based Release Versioning)
...
$UPDATE，更新版本号，用于修复新特性中的安全问题、回归和错误的兼容更新版本。（以前是$SECURITY，但有一个重要的递增规则。）

$PATCH，紧急补丁版本号，仅在需要生成紧急发布以修复关键问题时才会增加。

#### 新的 API

*Optional.orElseThrow()*，它是现有*get*方法的同义词，现在是首选的替代方法。

用于创建不可变集合的，*List.copyOf*, *Set.copyOf* 以及 *Map.copyOf*。同样，Steam 包下的 Collectors 类也增加了相关的方法 *toUnmodifiableList*, *toUnmodifiableSet* 和 *toUnmodifiableMap*。
